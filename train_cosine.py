...
import numpy as np
from biencoder_cos import BiEncoderRanker
...
def main(params):
    ...
    ent_embs, neighbor_list = process_candidate_data(model, device, entity_dictionary, n_candidates, params["debug"])
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc = "Processing minibatches")):
            ...
            target = torch.zeros(scores.shape[0],scores.shape[1])
            target[:,0] = 1
            target = target.float().to(device)
            probs = F.softmax(scores, dim=1)
        ...
        writer.add_scalar("Trainining loss 64 candidates / epoch", sum(loss_list)/len(loss_list), epoch_idx)
        
        writer.add_scalar("Trainining accuracy 64 candidates/ epoch", correct/total_samples, epoch_idx)
        ...
        writer.add_scalar("Validation loss 64 candidates/ epoch", validation_loss, epoch_idx)
        writer.add_scalar("Validation accuracy 64 candidates / epoch", validation_accuracy, epoch_idx)
        
        if validation_accuracy > best_val:
            best_val = validation_accuracy
            best_model = {'model': model.state_dict(),
                        'cosine_sim':reranker.cos.state_dict(),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_path",
        default="cosine_sim_64_candidates",
        type=str,
        required=False,
    )
